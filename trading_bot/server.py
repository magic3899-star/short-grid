...
def auto_listing_scanner():
    """4시간마다 신규상장 자동 스캔 및 워치리스트 추가 (최대 50개, 오래된 종목 밀어내기)"""
    SCAN_INTERVAL = 4 * 60 * 60  # 4시간
    MAX_DAYS = 30  # 30일 이내 상장 종목만
    MAX_WATCHLIST = 50  # 최대 워치리스트 개수
        ...
        try:
            ...
            for item in recent:
                ...
                if symbol not in watchlist:
                    info = get_symbol_info(symbol)
                    if info:
                        # 최대 개수 초과시 오래된 종목 삭제 (포지션/물타기 없는 것만)
                        while len(watchlist) >= MAX_WATCHLIST:
                            deleted = False
                            # 리스트 앞쪽이 오래된 종목
                            for old_symbol in watchlist[:]:
                                # 포지션 있으면 스킵
                                pos = get_position(old_symbol)
                                if pos and float(pos.get('positionAmt', 0)) != 0:
                                    continue
                                # 물타기 활성화면 스킵
                                if old_symbol in averaging_bots and averaging_bots[old_symbol].state.get('is_active'):
                                    continue
                                # 삭제
                                watchlist.remove(old_symbol)
                                if old_symbol in coin_data:
                                    del coin_data[old_symbol]
                                removed.append(old_symbol)
                                deleted = True
                                break
                            if not deleted:
                                # 삭제할 종목 없으면 중단
            ...
            if added or removed:
                ...
                msg = []
                if added:
                    msg.append(f'추가 {len(added)}개')
                if removed:
                    msg.append(f'삭제 {len(removed)}개')
                log(f'신규상장 스캔: {", ".join(msg)} (총 {len(watchlist)}개)', 'success')
            else:
                log(f'신규상장 스캔 완료 - 변동 없음 (총 {len(watchlist)}개)', 'info')
